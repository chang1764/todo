...
export function Column({ column, cards, onCardClick, onAddCard }: ColumnProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`flex flex-col rounded-xl p-4 min-w-96 h-fit max-h-[calc(100vh-150px)] border-2 transition-all ${
        isOver
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 pb-4 border-b border-slate-200">
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-gray-900 text-sm">{column.name}</h3>
          <p className="text-xs text-gray-500 mt-1">
            {columnCards.length} {columnCards.length === 1 ? '개' : '개'}
          </p>
        </div>
        <div className="flex gap-1 ml-2 flex-shrink-0">
          <Button
            variant="ghost"
            size="sm"
            className="h-8 w-8 p-0 text-slate-600 hover:text-slate-900 hover:bg-slate-200"
